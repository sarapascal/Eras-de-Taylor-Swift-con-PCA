...
## Vamos a realizar un Análisis de componentes principales en base a datos del Eras Tour de Taylor Swift. Utilizaremos una base de Kaggle que ya habíamos utilizado antes, pero con más profundidad.
...
## Agregamos la línea gráfica que queremos utilizar
swifty_colors <- list(
  lavanda = "#CDB4DB",
  morado  = "#6A0572",
  rosa    = "#FF69B4",
...
# Paletas por era (ajustadas según tu pedido)
swifty_era_pal <- list(
...
# Tema Swifty con 'family' explícito en títulos y subtítulos
theme_swifty <- function(base_family = "serif", base_size = 12) {
  ggplot2::theme_minimal(base_family = base_family, base_size = base_size) +
    ggplot2::theme(
      plot.background   = ggplot2::element_rect(fill = swifty_colors$crema, color = NA),
...
# Paleta corregida: Evermore = beige, Folklore = gris, Fearless = dorado (asegúrate de tener esta en tu swifty_era_pal)
fill_colors <- c(
...
# Comenzamos con nuestro análisis

## Con los nombres de la canciones definiremos las eras, haciendo un etiquetado de cada fila de la base para indicar a qué “era” (álbum) pertenece cada track. Esto nos va a permitir, agrupar y comparar el indicador PCA por era del tour más adelante.
...
df <- df %>%
  mutate(era = case_when(
    ...
    # Reputation
    track_name %in% c("...Ready For It?","Delicate","Don’t Blame Me","Look What You Made Me Do") ~ "Reputation",
    # folklore
    ...
    track_name %in% c("But Daddy I Love Him","So High School","Who’s Afraid of Little Old Me?",
...
## Revisando... para ver si todo está perfectly fine
## ¿Cuáles son las canciones sin era? (debe ser 0)
...
## Vamos a seleccionar las variables numéricas que van a entrar al PCA (en formato numérico, sin NA), después deberíamos filtrar outliers obvios (pero el principal era duración y no queremos borrar All too well) y luego vamos a estandarizar/escalar cada variable para que todas tengan media 0 y desvío estándar 1.

## Esta base tiene variables acústicas, así que utilizaremos esa información y también otras como duración y popularidad.
## En este caso, haremos la escala sin reducir por duración porque a pesar de que "All too well" se sale del rango normal o promedio porque es una parte importante del show. Veremos como se refleja esto después...
...
summary(df_variables[variables])  # después (medias ~0, sd ~1)
...
# Ejecutando PCA
## Como las variables ya están escaladas (z-score), no centramos/escalamos de nuevo
pca <- prcomp(X, center = FALSE, scale. = FALSE)

## Resumen rápido del PCA (varianza explicada por cada componente)
summary(pca)
...
# Comenzamos la representación visual
...
# Tabla rápida para consola
data.frame(PC = paste0("PC", seq_along(var_exp)),
           VarExp = round(var_exp, 4),
           AcumVar = round(acum_var, 4))

# Cargas (qué pesa en cada componente)
loadings <- pca$rotation

# Ver las cargas de los 3 primeros PCs (redondeadas)
print(round(loadings[, 1:3], 3))

# Variables que más pesan en PC1 (para nombrarlo)
sort(loadings[,1], decreasing = TRUE)

# Variables que más pesan en PC2
sort(loadings[,2], decreasing = TRUE)

#¿Qué son estos resultados?
##Si vemos cargas altas positivas en energy, loudness, tempo y negativas en acousticness → PC1 ≈ “Intensidad/Energía”. Y Si danceability y valence cargan alto en PC2 → PC2 ≈ “Bailabilidad/Positividad”. Speechiness e instrumentalness suelen matizar (más hablado vs. más instrumental).

# Dejémoslo en una tablita
knitr::kable(round(loadings[,1:3], 3), caption = "Cargas de las variables en PC1–PC3")

# Scores (coordenadas de cada canción en los PCs)
...
# Normalizamos el indicador principal (PC1) a escala 0–100 (para comunicarlo mejor)
rng <- range(df_scores$indicador_pc1, na.rm = TRUE)
df_scores <- df_scores %>%
  dplyr::mutate(indicador_0_100 = 100 * (indicador_pc1 - rng[1]) / (rng[2] - rng[1]))

## Ya que incluimos popularity dentro de las variables del PCA, el indicador mezcla perfil acústico con recepción del público.

# Top 10 canciones (mayor intensidad/energía según PC1)
top10 <- df_scores %>%
  dplyr::arrange(dplyr::desc(indicador_0_100)) %>%
  dplyr::select(track_name, indicador_0_100) %>%
  head(10)

knitr::kable(top10, caption = "Top 10 canciones por indicador (PC1 → 0–100)")
...
knitr::kable(indicador_era, caption = "Indicador promedio por era (PC1 → 0–100)")
...
# PCs y vectores de variables (diagnóstico rápido)
var_exp <- (pca$sdev^2) / sum(pca$sdev^2)
...
arrow_scale <- 4
titulo    <- "Relación entre observaciones y variables en el espacio PCA (PC1–PC2)"
...
### PC1 es alto en r paste(names(sort(loadings[,1], decreasing = TRUE)[1:3]), collapse = ", ") y bajo en r names(sort(loadings[,1], decreasing = FALSE)[1]); y lo interpretamos como ‘Intensidad/Energía’. El PC2 es alto en r paste(names(sort(loadings[,2], decreasing = TRUE)[1:3]), collapse = ", "), interpretado como ‘Bailabilidad/Positividad’.”

### Definimos el indicador principal como el score en PC1, normalizado a 0–100 para facilitar la lectura. Quedando el ranking de canciones y agregación por era.

### Las variables de entrada se estandarizaron (media 0, sd 1). No se filtró duración (se mantuvo ‘All Too Well (10 Minute Version)’ por su relevancia en el show).


# --- Heatmap de loadings PC1–PC3 ---
...
# Ordenamos variables por la magnitud de su contribución en PC1 (para visualizarlas)
...
loadings_df <- loadings_df %>%
  dplyr::mutate(
    ...
    PC = factor(
      ...
      labels = c(
        "PC1 = Intensidad/Energía",
        "PC2 = Bailabilidad/Positividad",
        "PC3 = Textura/Acústico–Vocal"
      )
    )
  )
...
ggplot(loadings_df, aes(x = PC, y = variable, fill = loading)) +
  ...
  labs(
    title = "Heatmap de cargas (loadings) en PC1–PC3",
    x = "Componente principal",
    y = "Variable"
  ) +
  eras_theme() +
...
ggplot(df_scores, aes(x = indicador_pc1, y = indicador_pc2)) +
  ...
  labs(title = "Mapa PC1 Intensidad/Energía vs PC2 	
Bailabilidad/Positividad de canciones del Eras Tour",
       x = "PC1 (Intensidad/Energía, según cargas)",
       y = "PC2 (Bailabilidad/Positividad, según cargas)") +
  eras_theme()
...
## Resumen del PCA:
### El PC1 Intensidad/Energía explica r scales::percent(var_exp[1]) y el PC2 Bailabilidad/Positividad r scales::percent(var_exp[2]) de la varianza; en conjunto suman r scales::percent(sum(var_exp[1:2])).
PC1 — Intensidad/Energía: altas cargas en energy, loudness, tempo y bajas en acousticness.
PC2 — Bailabilidad/Positividad: dominado por danceability y valence.
PC3 — Textura/Acústico–Vocal (o el nombre que definiste): matizado por speechiness, instrumentalness, liveness.

### Indicador compuesto:
Definimos el indicador principal como el score en PC1, normalizado a 0–100. Presentamos el ranking de canciones y la agregación por era.

### Decisiones metodológicas:
(i) Estándar z–score en features; (ii) No se filtró la duración por la relevancia de “All Too Well (10 Minute Version)”; (iii) La inclusión/exclusión de popularity dentro del PCA se documenta.

### Limitaciones:
Los cambios de setlist y versiones (Taylor’s Version) pueden afectar comparabilidad si se amplía la base.
Las representaciones no son tan alineadas con la línea gráfica, pero se intentó lo más que se pudo.
Agradezco nuevamente a https://www.kaggle.com/datasets/yukawithdata/taylor-swift-the-eras-tour-official-setlist-data por poner a disposición esta increíble base de datos.
Cierro este curso muy agradecida y entusiasta por lo mucho aprendido, si bien esta actividad tuvo una dificultad mayor, pude usar una base que me interesa y eso me permitió trabajar con más determinación en llegar a mejores resultados (a pesar de que pasé más tiempo intentando arreglar los colores de las barras por era que en el resto). Gracias por leer! 
